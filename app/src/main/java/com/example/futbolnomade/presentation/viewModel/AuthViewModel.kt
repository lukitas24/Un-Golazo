...
class AuthViewModel(
            ...
            try {
                ...
                if (firebaseUser != null) {
                    try {
                        ...
                        Log.e("NOTIFICACION_CHECK", "⚠️ Error menor en reparación de perfil: ${e.message}")
                    }
                }
                ...
                Log.e("NOTIFICACION_CHECK", "❌ Error en login Auth: ${exception.message}")
                accesoDesbloqueado = false
                
                val firebaseException = exception as? FirebaseAuthException
                val mensaje = when (firebaseException?.errorCode) {
                    "ERROR_INVALID_CREDENTIAL", "ERROR_WRONG_PASSWORD", "ERROR_USER_NOT_FOUND" -> "Email o contraseña incorrectos."
                ...
                onResult(AuthResult.Error(mensaje))
            }
        }
    }
    ...
    fun registrar(nombre: String, email: String, password: String, onResult: (AuthResult) -> Unit) {
        ...
        viewModelScope.launch {
            try {
                Log.i("NOTIFICACION_CHECK", "📝 Registrando usuario: $cleanEmail")
                val authResult = auth.createUserWithEmailAndPassword(cleanEmail, password).await()
                val firebaseUser = authResult.user ?: throw IllegalStateException("Error al crear usuario")
                ...
                Log.e("NOTIFICACION_CHECK", "❌ Error en registro: ${exception.message}")
                accesoDesbloqueado = false
                
                val firebaseException = exception as? FirebaseAuthException
                val mensaje = when (firebaseException?.errorCode) {
                    "ERROR_EMAIL_ALREADY_IN_USE" -> "El email ya está en uso."
                    "ERROR_WEAK_PASSWORD" -> "La contraseña es muy débil."
                    else -> exception.message ?: "No se pudo crear la cuenta."
                }
                onResult(AuthResult.Error(mensaje))
            }
        }
    ...
    private suspend fun registrarDispositivoActual() {
        ...
        try {
            Log.i("NOTIFICACION_CHECK", "📡 Registrando token FCM para UID: $uid")
    ...
    fun loginWithBiometricCredentials(credentials: BiometricCredentials, onResult: (AuthResult) -> Unit) {
        viewModelScope.launch {
            try {
                auth.signOut()
                auth.signInWithEmailAndPassword(credentials.email.trim().lowercase(), credentials.password).await()
                actualizarUsuarioDesdeFirebase()
                registrarDispositivoActual()
                accesoDesbloqueado = true
                onResult(AuthResult.Success)
                ...
                accesoDesbloqueado = false
                onResult(AuthResult.Error(exception.message ?: "Error biométrico"))
            }
        }
    }
}
